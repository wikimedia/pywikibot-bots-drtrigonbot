...
<html>
<head>
  <title>DrTrigonBot checksum checker</title>
</head>
<body>
<p><span style="font-family:sans-serif">
DrTrigonBot checksum checker<br><br><br>
%(output)s<br>
<small>DrTrigonBot checksum checker, written by <a href="https://wiki.toolserver.org/view/User:DrTrigon">DrTrigon</a>. 
...
REQUEST_getSections_page	= ("de.wikipedia.org", "/w/api.php?action=parse&page=%s&prop=sections&format=xml")
REQUEST_getPageSection		= ("de.wikipedia.org", "/w/api.php?action=query&prop=revisions&titles=%s&rvprop=timestamp|user|comment|content&rvsection=%s&format=xml")
...
def _apiread(request, parent, content, values):
	...
	dh = _GetData(parent, content, values)		# Create the handler
	...
	if (values == []): 
		...
		m = re.search('^(=+)(.*?)(=+)(?=\s)', dh.chars, re.M)
		if m:	section_name = m.groups()[1].strip()
		else:	section_name = "?"

		return (section_name, dh, text)
	return dh.data
...
def displayhtmlpage(form):
	...
	for i, title in enumerate(titles):
		...
		request = REQUEST_getSections_page[1] % urllib.quote(title)

		test = _apiread((REQUEST_getSections_page[0], request), 'sections', 's', ['line', 'number'])
		test = [item[0] for item in test]
		if section in test: section = str( test.index(section)+1 )

		#request = REQUEST_getPageSection[1] % ("Benutzer Diskussion:Debianux", "Automatische Begrüssung")
		#request = REQUEST_getPageSection[1] % (title[0].encode(textfile_encoding), title[1])
                #request = REQUEST_getPageSection[1] % (urllib.quote(title[0].encode(textfile_encoding)), urllib.quote(title[1].encode(textfile_encoding)))
                request = REQUEST_getPageSection[1] % (urllib.quote(title), urllib.quote(section))

		(section_name, dh, text) = _apiread((REQUEST_getPageSection[0], request), 'revisions', 'rev', [])

		result += "Page: <i>%s</i> / Section: <i>%s (%s)</i>" % (title, section_name.encode(textfile_encoding), section)
		result += "<br>"
		result += "Old checksum: %s" % chksum[i]
		result += "<br>"

		new_chksum = hashlib.md5(dh.chars.encode('utf8').strip()).hexdigest()
		result += "New checksum: %s" % new_chksum
		result += "<br>"
		result += "<b>Section changed: %s</b>" % (not (chksum[i] == new_chksum))

		#result += "<br>"
		#result += "<div style=\"padding:20px; border:thin solid gray; margin:25px\">"
		#result += "<small>"
		##result += re.sub('\n','<br>',text).encode('utf8')
